...
public class AdViewPager extends ViewPager {
    ...
    private int mSlidingDistanceY = 50; // y轴方向最小滑动距离
    ...
    public AdDurationScroller scroller;
    ...
    private void setViewPagerScroller() {
        try {
            ...
            scroller = new AdDurationScroller(getContext(), (Interpolator)interpolatorField.get(null));
    ...
    private static class MyHandler extends Handler {
        ...
        private final WeakReference<AdViewPager> autoScrollViewPager;

        public MyHandler(AdViewPager autoScrollViewPager) {
            this.autoScrollViewPager = new WeakReference<AdViewPager>(autoScrollViewPager);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SCROLL_WHAT:
                    AdViewPager pager = this.autoScrollViewPager.get();
                    if (pager != null) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                float tmpX = ev.getX();
                float tmpY = ev.getY();
                if(Math.abs(tmpY - mDownY) > Math.abs(tmpX-mDownX)
                        && Math.abs(tmpY - mDownY) > mSlidingDistanceY){
                    getParent().requestDisallowInterceptTouchEvent(false);
                } else {
                    getParent().requestDisallowInterceptTouchEvent(true);
                }
